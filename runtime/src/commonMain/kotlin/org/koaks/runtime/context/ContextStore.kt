...
class ContextAccessException(val ref: ContextRef, val requester: AgentId?) :
...
class ContextStore {
    ...
    fun put(messages: List<Message>, scope: ContextScope = ContextScope.GLOBAL, owner: AgentId? = null): ContextRef {
     ...
     */
    fun resolve(ref: ContextRef, requester: AgentId?): List<Message> {
    ...
    private fun checkAccess(block: ContextBlock, requester: AgentId?) {
    ...
    private fun address(scope: ContextScope, owner: AgentId?, parent: ContextRef?, messages: List<Message>): String {
