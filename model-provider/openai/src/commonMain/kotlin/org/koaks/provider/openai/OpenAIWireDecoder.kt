...
/**
 * Stateful decoder for OpenAI's Chat Completions stream.
 *
 ...
 */
class OpenAIWireDecoder : WireDecoder<OpenAIChatResponse> {
    ...
    override fun accept(chunk: OpenAIChatResponse): List<ModelEvent> {
        val events = mutableListOf<ModelEvent>()

        chunk.error?.let { err ->
            failed = AgentError.ModelError(
                message = err.message ?: "openai error ${err.code}",
                retriable = false,
            )
