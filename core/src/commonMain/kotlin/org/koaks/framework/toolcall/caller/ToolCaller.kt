...
object ToolCaller : IOutCaller {
    ...
    override suspend fun call(toolname: String, json: String): String {
        return ToolManager.getTool(toolname)?.let { tool ->
            if (tool.toolType == ToolType.ANNOTATION) {
                ...
                tool.toolImplementation!!.executeJson(json) ?: "execute success"
            }
        } ?: run {
    ...
    private fun parseToolArguments(tool: ToolDefinition, rawJson: String?): List<Any> {
        ...
        return try {
            ...
            logger.error(e) { "Failed to parse arguments for ${tool.toolName}" }
            emptyList()
        }
    }
