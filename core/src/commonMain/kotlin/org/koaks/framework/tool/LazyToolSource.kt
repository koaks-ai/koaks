 ...
 * time (design §5.1). The DSL is synchronous, but some tool sets (notably MCP) need
 * a suspend handshake + `tools/list` to discover. The builder registers a source;
 * [AgentRunner] resolves it once on the first run and appends the results to the
